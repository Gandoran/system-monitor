...
// --- STRUTTURE PER MAPPARE IL DATABASE WMI DI WINDOWS ---
...
impl IntelStrategy {
    pub fn new() -> Self {
        let mut identity = GpuIdentity {
            ...
            vram_total: 0,
            power_max_w: 0.0, // Limite Kernel
            max_mhz: 0,       // Limite Kernel
        };

        // CONNESSIONE WMI STATICA (Viene eseguita 1 sola volta all'avvio)
        if let Ok(com_con) = COMLibrary::new() {
            if let Ok(wmi_con) = WMIConnection::new(com_con) {
                ...
                if let Ok(controllers) = wmi_con.raw_query::<Win32VideoController>(query) {
                    // Cerchiamo la prima scheda che contiene "Intel" (Iris o Arc)
                    if let Some(intel_gpu) = controllers.into_iter().find(|c| c.name.to_lowercase().contains("intel")) {
                        ...
                        // 1. FIX: Salviamo i Watt massimi e i Mhz massimi dal nostro "Dizionario"
...
impl GpuStrategy for IntelStrategy {
    fn read(&self) -> GpuMetrics {
        ...
        // 4. FIX WATT E MHZ: Estrapolazione intelligente!
        // Stimiamo un consumo minimo "idle" (es. 5W e 300MHz) e lo facciamo salire proporzionalmente al carico %
        let power_draw_w = if gpu_usage > 0.0 {
            5.0 + ((gpu_usage / 100.0) * (self.identity.power_max_w - 5.0))
        } else {
            2.0 // Consumo a riposo assoluto
        };

        let gpu_mhz = if gpu_usage > 0.0 {
            300 + ((gpu_usage / 100.0) * (self.identity.max_mhz as f32 - 300.0)) as u32
        } else {
            300 // Clock a riposo assoluto
        };

        GpuMetrics {
            gpu_usage, 
            vram_used, 
            power_draw_w,   // <-- Ora è popolato!
            gpu_mhz,        // <-- Ora è popolato!
            fan_speed_pct,  // <-- Ora è popolato!
            
            gpu_temp: 0.0, // Solo questo resta mockato
            gpu_max_temp: 0.0,
            gpu_active,
