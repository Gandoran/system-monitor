...
export interface RustPayload {
    cpu_stats: { cpu_name:string, cpu_usage: number, cores_load:number[],frequency:number,physical_cores:number };
    cpu_temp: { cpu_temp: number, max_temp:number };
    ram_stats: { ram_used: number, ram_total: number, ram_available: number, swap_used: number, swap_total: number, };
    ...
    disk_stats: {disk_write:number,disk_read:number,disk_total_memory:number,disk_used_memory:number,disk_use:number
        disk_name:string, file_system:string,disk_type:string};
